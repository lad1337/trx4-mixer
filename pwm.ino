...
const int pwmPIN[] = {GEAR_LIGHT_PIN, DIFFERENTIAL_PIN, WINCH_PIN}; // an array to identify the PWM input pins (the array can be any length)
...
void pciSetup(byte pin) {
  pinMode(pin, INPUT);
  int maskbit = digitalPinToPCMSKbit(pin);
  int registerbit = digitalPinToPCICRbit(pin);
  Serial.print("PCI setup pin: ");
  Serial.print(pin);
  Serial.print(" -> ");
  Serial.print(maskbit);
  Serial.print(" -> ");
  Serial.println(registerbit);
  byte mask = *digitalPinToPCMSK(pin) |= bit (maskbit);  // enable pin
  byte interupts = *digitalPinToPCICR(pin) |= bit (registerbit);
  PCIFR  |= bit (digitalPinToPCICRbit(pin));                   // clear any outstanding interrupt
  /*
    Serial.print("Mask: ");
    Serial.println(mask, BIN);
    Serial.print("interupts: ");
    Serial.println(interupts, BIN);
  */
}
...
void pwmPIN_to_port() {
  for (int i = 0; i < num_ch; i++) {
    ...
    pwmPIN_reg[i] = bit (digitalPinToPCMSKbit(pwmPIN[i]));
  }
}
